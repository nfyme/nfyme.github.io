...
class TiledLayer {
  ...
  getTileCoordinatesInViewport() {
    ...
    const buffer = this.tiler.options.viewportBuffer || 0;

    const viewportRight = this.tiler.pswp.viewportSize.x;
    const viewportBottom = this.tiler.pswp.viewportSize.y;

    const leftTileX = getTileCoordinateByPosition(-tileLeft - buffer, tileWidth, this.numXTiles);
    const rightTileX = getTileCoordinateByPosition(viewportRight - tileLeft + buffer, tileWidth, this.numXTiles);

    const topTileY = getTileCoordinateByPosition(-tileTop - buffer, tileHeight, this.numYTiles);
    const bottomTileY = getTileCoordinateByPosition(viewportBottom - tileTop + buffer, tileHeight, this.numYTiles);
...
const defaultOptions = {
  ...
  // Насколько за пределы видимой области (в пикселях экрана) заранее
  // подгружать тайлы, чтобы при панорамировании не было видно подгрузку по краям
  viewportBuffer: 0
};
